...
export type ThemeId = 'colorwars' | 'cell' | 'origami' | 'stainedglass';
...
// ═══════════════════════════════════════════
// THEME 4: 색깔 전쟁 (Color Wars)
// ═══════════════════════════════════════════
export const COLOR_WARS_THEME: ThemeConfig = {
  id: 'colorwars',
  name: 'Color Wars',
  nameKo: '색깔 전쟁',
  background: 0xFAFAFA,
  gridCellColor: 0xF5F5F5,
  gridCellAlpha: 0.8,
  gridLineColor: 0xE0E0E0,
  gridLineAlpha: 0.9,
  pathColor: 0xBDBDBD,
  pathAlpha: 0.6,
  pathLineColor: 0xBDBDBD,
  pathLineWidth: 8,
  hudBgColor: 0xF0F0F0,
  hudBgAlpha: 0.95,
  hudTextColor: '#333333',
  hudAccentColor: '#FF1744',
  panelBgColor: 0xEEEEEE,
  panelBgAlpha: 0.97,
  towerVisuals: {
    archer: { color: 0xFFEB3B, glowColor: 0xFFC107, shape: 'circle', outlineWidth: 2, outlineColor: 0xF9A825, alpha: 0.95 },
    cannon: { color: 0xFF1744, glowColor: 0xD50000, shape: 'circle', outlineWidth: 2, outlineColor: 0xB71C1C, alpha: 0.95 },
    slow:   { color: 0x448AFF, glowColor: 0x1565C0, shape: 'circle', outlineWidth: 2, outlineColor: 0x0D47A1, alpha: 0.95 },
  },
  enemyVisuals: {
    goblin: { color: 0x9E9E9E, shape: 'circle',   radius: 10, alpha: 0.9, outlineColor: 0x757575, outlineWidth: 1 },
    wolf:   { color: 0x424242, shape: 'triangle', radius: 10, alpha: 0.9, outlineColor: 0x212121, outlineWidth: 1 },
    troll:  { color: 0xF5F5F5, shape: 'circle',   radius: 16, alpha: 0.9, outlineColor: 0x9E9E9E, outlineWidth: 2 },
  },
  projectileColor: 0xFF1744,
  projectileGlow: 0xFFEB3B,
  mergeParticleColors: [0xFFEB3B, 0xFF1744, 0x448AFF],
  deathParticleColor: 0x9E9E9E,
  hitFlashColor: 0xFFFFFF,

  drawTower: (gfx, type, tier, size) => {
    // T1 sizes: 24, T2: 32, T3: 40 (radius)
    const radii = [24, 32, 40];
    const r = radii[tier - 1];

    const t1Colors: Record<TowerType, number> = { archer: 0xFFEB3B, cannon: 0xFF1744, slow: 0x448AFF };
    const t2Colors: Record<TowerType, number> = { archer: 0xFFC107, cannon: 0xD50000, slow: 0x1565C0 };
    const t3Colors: Record<TowerType, number> = { archer: 0xF9A825, cannon: 0xB71C1C, slow: 0x0D47A1 };

    const mainColor = tier === 1 ? t1Colors[type] : tier === 2 ? t2Colors[type] : t3Colors[type];
    const coreColor = tier === 1 ? t1Colors[type] : tier === 2 ? t2Colors[type] : t3Colors[type];

    if (tier === 3) {
      // T3: outer glow ring 3px
      gfx.lineStyle(3, mainColor, 0.5);
      gfx.strokeCircle(0, 0, r + 5);
      gfx.fillStyle(mainColor, 0.1);
      gfx.fillCircle(0, 0, r + 5);
    }

    if (tier >= 2) {
      // T2+: subtle halo
      gfx.fillStyle(mainColor, 0.15);
      gfx.fillCircle(0, 0, r + 3);
      // Small particles at cardinal points
      for (let i = 0; i < 4; i++) {
        const angle = (i / 4) * Math.PI * 2;
        const px = Math.cos(angle) * (r + 6);
        const py = Math.sin(angle) * (r + 6);
        gfx.fillStyle(mainColor, 0.6);
        gfx.fillCircle(px, py, 2);
      }
    }

    // Main circle body
    gfx.fillStyle(mainColor, 0.9);
    gfx.fillCircle(0, 0, r);
    ...
    gfx.fillCircle(-r * 0.25, -r * 0.25, r * 0.3);
    gfx.fillStyle(coreColor, 0.8);
    gfx.fillCircle(0, 0, r * 0.4);
    gfx.fillStyle(0xFFFFFF, 0.7);
  ...
  drawEnemy: (gfx, type) => {
    if (type === 'goblin') {
      // Grey circle
      gfx.fillStyle(0x9E9E9E, 0.9);
      gfx.fillCircle(0, 0, 10);
      gfx.lineStyle(1, 0x757575, 0.6);
      ...
      // Dark grey triangle
      const r = 10;
      gfx.fillStyle(0x424242, 0.9);
      gfx.fillTriangle(0, -r, -r, r, r, r);
      gfx.lineStyle(1, 0x212121, 0.7);
      gfx.strokeTriangle(0, -r, -r, r, r, r);
      // White eye
      gfx.fillStyle(0xF5F5F5, 0.9);
      gfx.fillCircle(0, 0, 2);
    } else {
      // Troll: bleach white large circle, grey border only
      const r = 16;
      gfx.fillStyle(0xF5F5F5, 0.95);
      gfx.fillCircle(0, 0, r);
      gfx.lineStyle(2, 0x9E9E9E, 0.8);
      gfx.strokeCircle(0, 0, r);
      // Hollow eyes (just grey circles)
  ...
  drawGrid: (gfx, x, y, w, h, isPath) => {
    if (isPath) {
      gfx.fillStyle(0xBDBDBD, 0.35);
    } else {
      gfx.fillStyle(0xF5F5F5, 0.7);
    }
    gfx.fillRect(x, y, w, h);
    gfx.lineStyle(1, 0xE0E0E0, 0.9);
    gfx.strokeRect(x, y, w, h);
  },

  drawPath: (gfx, waypoints) => {
    // Paint-flow grey path
    gfx.lineStyle(10, 0xBDBDBD, 0.5);
    gfx.beginPath();
    gfx.moveTo(waypoints[0].x, waypoints[0].y);
    for (let i = 1; i < waypoints.length; i++) {
      gfx.lineTo(waypoints[i].x, waypoints[i].y);
...
const THEMES: Record<ThemeId, ThemeConfig> = {
  colorwars: COLOR_WARS_THEME,
  cell: CELL_THEME,
  origami: ORIGAMI_THEME,
  stainedglass: STAINEDGLASS_THEME,
};

let currentTheme: ThemeId = 'colorwars';
